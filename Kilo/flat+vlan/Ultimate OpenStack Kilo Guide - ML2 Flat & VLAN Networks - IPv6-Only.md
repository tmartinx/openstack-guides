# Ultimate OpenStack Kilo Guide

This is a Quick Guide to deploy OpenStack Kilo on top of Ubuntu 14.04.2, it is IPv6-Only (almost)! But, it can be used to deploy a IPv4-Only environment, just replace the IPs with `sed`.

It is compliant with OpenStack's official documentation (docs.openstack.org/kilo).
...
Get it here (example):
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/upstream-router/etc/network/interfaces > /etc/network/interfaces
...
OpenStack Kilo is now compatible with an *upstream router* running radvd.

Get it here (example):

    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/upstream-router/etc/radvd.conf > /etc/radvd.conf
...
Run:
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/controller/etc/hosts > /etc/hosts
...
Get your Controller Node network interfaces file (example):
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/controller/etc/network/interfaces > /etc/network/interfaces
...
Reconfigure MySQL by downloading a new `my.cnf`:
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/etc/mysql/my.cnf > /etc/mysql/my.cnf
...
Create/reset the required databases:
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/root/mysql-dbs.sql | mysql -u root -p

### Documentation reference

 * http://docs.openstack.org/kilo/install-guide/install/apt/content/ch_basic_environment.html#basics-packages
 * http://docs.openstack.org/kilo/install-guide/install/apt/content/ch_basic_environment.html#basics-messaging-server
...
Download a new `keystone.conf` (it is based on Kilo Ubuntu packages, plus a few changes):

    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/etc/keystone/keystone.conf > /etc/keystone/keystone.conf
...
More Keystone stuff:
    ...
    wget https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/root/keystone_basic.sh

    wget https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/root/keystone_endpoints_basic.sh
...
Download the `NOVA Resource Configuration` file of your `admin` user:
    ...
    wget https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/root/.novarc
...
## 6.1. Cinder API / endpoint access:
    ...
    curl -s https://raw.githubusercontent.com/tmartinx/openstack-guides/master/Kilo/flat+vlan/controller/etc/cinder/cinder.conf > /etc/cinder/cinder.conf
...
- OpenStack Kilo Documentation for Ubuntu:
...
When using OpenStack Kilo, there is only one place that IPv4 is still required: For the Metadata Network. So, the tenants still needs IPv4 connectivity, just to put Metadata to work. This will be required until `cloud-init`, and OpenStack itself, doesn't provides "Metadata over IPv6" support.

*NOTE: Now with Kilo, there is a new Metadata option, called "Config Drive", it seems to be possible to use it in place of current IPv4 subnet (169.254.169.254). I'll try it soon!*
...
You can easily see the differences between the files from the guide, with the files from the packages, just use `diff -Nru /etc/neutron/neutron.conf kilo-guide-example-neutron.conf`.
